...
interface FavAddingDao {
    @Query("SELECT * FROM fav_adding")
    fun getFavAdding(): Flow<List<FavAdding>>

    @Query("SELECT EXISTS(SELECT 1 FROM fav_addings WHERE alphabet_id = :alphabetId LIMIT 1)")
     ...
     */
    @Transaction
    @Query("SELECT * FROM alphabets WHERE id IN (SELECT DISTINCT(alphabet_id) FROM fav_adding)")
    ...
    @Delete
    suspend fun deleteGardenPlanting(favAdding: FavAdding)
}
